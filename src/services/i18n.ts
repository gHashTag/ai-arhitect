...
export async function initI18n(): Promise<void> {
  ...
  const ltTranslations = JSON.parse(fs.readFileSync(path.join(localesDir, 'lt.json'), 'utf8'));
  const ruTranslations = JSON.parse(fs.readFileSync(path.join(localesDir, 'ru.json'), 'utf8'));
...
export function determineLanguage(languageCode: string | undefined): SupportedLanguage {
  if (!languageCode) {
...
export function t(language: SupportedLanguage, key: string, options?: any): string {
